...
func set_danger():
	...
	for i in num_rays:
		...
		global_position + Vector3(0,0.5,0) + ray_directions[i].rotated(Vector3.UP, rotation.y) * look_ahead, 1))
		if result:
			if is_chasing:
				var player_group
				for group in get_groups():
					if str(group).begins_with("player"):
						player_group = str(group)
						break
				if result.collider.is_in_group(player_group):
					danger[i] = 1.0
			else:
				danger[i] = 1.0
		else:
